...
const Home = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (isConnected) {
        // If connected, fetch from server and save to AsyncStorage
        await fetchManufacturers();
        await fetchCarModel();
        await fetchCarVarient();
        await fetchCarYears();
        await fetchCarColors();
        await fetchFuelTypes();
        await fetchTransmissionsTypes();
        await fetchEngineCapacity();
        await fetchRegistrationCity();
        setLoading(false);
      } else {
        // If not connected, data is loaded from AsyncStorage in the context
        setLoading(false);
      }
    };
